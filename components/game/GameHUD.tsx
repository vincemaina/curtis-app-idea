...
export default function GameHUD({
  ...
  // Track whether the user has ever locked (so we can differentiate "first load" vs "paused")
  const hasStartedRef = useRef(false)
  useEffect(() => {
    if (isLocked) hasStartedRef.current = true
  }, [isLocked])

  const isPaused  = !isLocked && !chatOpen && hasStartedRef.current
  const isPreGame = !isLocked && !chatOpen && !hasStartedRef.current

  return (
    <>
      {/* ── Top bar (hidden when chat covers it, but still in DOM) ── */}
      <div
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Finish button only shown when cursor is free (paused or pre-game) */}
          {!isLocked && (
            <button
              onClick={onFinish}
      ...
      {newlyCompleted.length > 0 && (
        <div
          className="absolute top-14 left-1/2 -translate-x-1/2 px-4 py-2 rounded-xl animate-slide-up"
          style={{
            zIndex: 50,
          ...
          {newlyCompleted.map(oid => {
            ...
            return (
              <div key={oid} className="flex items-center gap-2 text-sm text-green-400">
                <span>✓</span>
                <span>
                  {obj?.type === 'primary' ? '🛒 Picked up' : '⭐ Objective complete'}:{' '}
      ...
      <div className="absolute bottom-5 left-4 max-w-xs" style={{ zIndex: 50 }}>
        ...
        >
          <div className="text-xs font-semibold text-white/45 uppercase tracking-widest mb-2">
            Shopping List
          </div>
      ...
      {/* ── Controls hint — bottom-right (only when actively playing) ── */}
      {isLocked && (
        <div className="absolute bottom-5 right-4" style={{ zIndex: 20 }}>
          <div
          ...
          >
            ...
            <div><span className="text-white/80 font-medium">E</span> <span className="text-white/40">Talk / Pick up</span></div>
            <div><span className="text-white/80 font-medium">Esc</span> <span className="text-white/40">Pause</span></div>
          </div>
        </div>
      )}
      ...
      {isPreGame && (
        <div
          ...
          style={{ zIndex: 20, background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(2px)' }}
        >
          <div
            className="text-center px-8 py-7 rounded-2xl max-w-sm"
            style={{ background: 'rgba(15,15,26,0.92)', border: '1px solid rgba(124,92,252,0.4)' }}
          >
            <div className="text-4xl mb-3">{scenario.emoji}</div>
            <h2 className="text-white font-bold text-xl mb-1">{scenario.name}</h2>
            <p className="text-white/50 text-sm mb-5 leading-relaxed">
              {scenario.setting.slice(0, 110)}…
            </p>
            <div
              className="text-white font-semibold py-2.5 px-6 rounded-xl inline-block cursor-pointer mb-4"
              style={{ background: '#7c5cfc' }}
            >
              Click to play
            </div>
            <div className="text-xs text-white/35 space-y-1">
              <div>WASD · move &nbsp;|&nbsp; Mouse · look &nbsp;|&nbsp; E · talk / pick up</div>
            </div>
          </div>
        </div>
      )}

      {/* ── Pause menu (pressed Esc mid-game) ── */}
      {isPaused && (
        <div
          className="absolute inset-0 flex items-center justify-center"
          style={{ zIndex: 20, background: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)' }}
        >
          <div
            className="text-center px-8 py-7 rounded-2xl max-w-xs w-full"
            style={{ background: 'rgba(15,15,26,0.96)', border: '1px solid rgba(124,92,252,0.4)' }}
          >
            <div className="text-2xl mb-1">⏸</div>
            <h2 className="text-white font-bold text-lg mb-1">Paused</h2>
            <p className="text-white/40 text-xs mb-5">
              {completedObjectiveIds.filter(id =>
                scenario.objectives.find(o => o.id === id && o.type === 'primary')
              ).length}{' '}
              / {primary.length} items found
            </p>

            <div className="space-y-2">
              <div
                className="text-white font-semibold py-2 px-6 rounded-xl cursor-pointer w-full text-sm text-center"
                style={{ background: '#7c5cfc' }}
              >
              ...
              <button
                onClick={onFinish}
                className="w-full py-2 px-6 rounded-xl text-sm font-semibold text-white/70 hover:text-white border border-white/15 hover:border-white/30 transition-colors"
              >
                Finish &amp; Analyse
              </button>
            </div>
